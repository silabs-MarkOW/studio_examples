...
#include "voice.h"
#include "em_gpio.h"
#include "sl_emlib_gpio_init_copy_to_fifo_config.h"
#include "sl_emlib_gpio_init_isr_callback_config.h"
...
#define CIRCULAR_BUFFER_SIZE    (MIC_SAMPLE_BUFFER_SIZE * 50)
...
static uint32_t frames, max_frames, min_frames;
...
uint32_t count_mic_buffer_ready = 0;
uint32_t count_voice_send_data = 0;
uint32_t count_cb_errors = 0;
...
void voice_start(void)
{
  ...
  voice_running = true;
  count_cb_errors = 0;
  count_mic_buffer_ready = 0;
  count_voice_send_data = 0;
}
...
void voice_stop(void)
{
  ...
  voice_running = false;
#define P(X) app_log(#X ": %ld\r\n",X)
  P(count_cb_errors);
  P(count_mic_buffer_ready);
  P(count_voice_send_data);
  P(max_frames);
  P(min_frames);
#undef P
}
 ...
 ******************************************************************************/
SL_WEAK bool voice_transmit(uint8_t *buffer, uint32_t size)
{
  (void)buffer;
  (void)size;
  return true;
...
void voice_set_sample_rate(sample_rate_t sample_rate)
{
  if ((sample_rate == sr_16k) || (sample_rate == sr_8k)) {
    ...
    voice_config.sampleRate = 48;
    app_log("set sample rate: %d\r\n", sample_rate);
  } else {
    app_log("ignoring sample rate: %d\r\n", sample_rate);
  }
...
void voice_set_channels(uint8_t channels)
{
  if ((channels > 0) && (channels <= MIC_CHANNELS_MAX)) {
    ...
    app_log("set channels: %d\r\n", channels);
  } else {
      app_log("ignore channels: %d\r\n", channels);
  }
...
GPIO_PinOutSet(SL_EMLIB_GPIO_INIT_COPY_TO_FIFO_PORT, SL_EMLIB_GPIO_INIT_COPY_TO_FIFO_PIN);
  ...
  app_assert(err == cb_err_ok,
             ...
             (int)err);
  GPIO_PinOutClear(SL_EMLIB_GPIO_INIT_COPY_TO_FIFO_PORT, SL_EMLIB_GPIO_INIT_COPY_TO_FIFO_PIN);
...
static void voice_send_data(void)
{
  cb_err_code_t cb_error = cb_err_ok;
  static uint8_t buffer[MIC_SEND_BUFFER_SIZE];
...
if(!pending) {
  ...
  if ( cb_error == cb_err_ok ) {
    pending = voice_transmit(buffer, MIC_SEND_BUFFER_SIZE);
    event_send = true;
    if(!pending) {
    count_voice_send_data += MIC_SEND_BUFFER_SIZE;
    }
  } else {
      count_cb_errors++;
  }
...
static void mic_buffer_ready(const void *buffer, uint32_t n_frames)
{
  // app_log("mic_buffer_ready(n_frames:%ld)\r\n",n_frames);
  sample_buffer = (int16_t *)buffer;
  frames = n_frames;
  count_mic_buffer_ready += frames << 1;
  event_process = true;
  GPIO_PinOutToggle(SL_EMLIB_GPIO_INIT_ISR_CALLBACK_PORT, SL_EMLIB_GPIO_INIT_ISR_CALLBACK_PIN);
}
